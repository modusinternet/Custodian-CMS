...
if(($json_a["admin"]["sub"]["backup_restore"] ?? null) < 1) {
	echo "Access denied.";
	die();
}
...
<html lang="{CCMS_LIB:_default.php;FUNC:ccms_lng}">
	<head>
		<title><?= $_SERVER["SERVER_NAME"];?> | User | Admin | Backup - Restore</title>
	...
	<script nonce="{CCMS_LIB:_default.php;FUNC:ccms_csp_nounce}">
		let navActiveItem = ["nav-admin","nav-admin-backup_restore"];
		let navActiveSub = [];
