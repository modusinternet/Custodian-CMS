...
if(($json_a["admin"]["sub"]["blacklist"] ?? null) < 1) {
	echo "Access denied.";
	die();
}
...
<html lang="{CCMS_LIB:_default.php;FUNC:ccms_lng}">
	<head>
		<title><?= $_SERVER["SERVER_NAME"];?> | User | Admin | Blacklist</title>
	...
	<script nonce="{CCMS_LIB:_default.php;FUNC:ccms_csp_nounce}">
		let navActiveItem = ["nav-admin","nav-admin-blacklist"];
		let navActiveSub = [];
		let navActiveW3schoolsItem = [];
	</script>
	<body>
		<main style="padding:20px 20px 20px 0">
			<h1 style="border-bottom:1px dashed var(--cl3)">Admin | Blacklist</h1>
