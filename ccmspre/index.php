		...
		if($duration >= $CFG["COOKIE_SESSION_EXPIRE"]) {
			...
			if(isset($_SESSION["USER_ID"])) {
				...
				if($CFG["LOG_EVENTS"] === 1) {
					...
					$qry->execute(array(':date' => time(), ':ip' => $_SERVER["REMOTE_ADDR"], ':url' => $_SERVER["REQUEST_URI"], ':log' => "User ID (".$_SESSION["USER_ID"].") session expired, redirected to the login page.\n".$_SERVER["HTTP_USER_AGENT"]));
				}
				//*/
	...
	if(isset($_SESSION["HTTP_USER_AGENT"])) {
		...
		if($_SESSION["HTTP_USER_AGENT"] != md5($_SERVER["HTTP_USER_AGENT"])) {
			...
			if($CFG["LOG_EVENTS"] === 1) {
				...
				$qry = $CFG["DBH"]->prepare("INSERT INTO `ccms_log` (`id`, `date`, `ip`, `url`, `log`) VALUES (NULL, :date, :ip, :url, :log);");
				$qry->execute(array(':date' => time(), ':ip' => $_SERVER["REMOTE_ADDR"], ':url' => $_SERVER["REQUEST_URI"], ':log' => "User ID (".$_SESSION["USER_ID"].") under possible session highjacking attempt.  The Session and Server HTTP_USER_AGENT's do not match.  Therefore, the session was deleted and the user was redirected to the login page.\n".$_SERVER["HTTP_USER_AGENT"]."\nNOTE: This type of error can also be accidentally generated by users testing pages. (Making media source and resolution changes in developer mode.)"));
			}
